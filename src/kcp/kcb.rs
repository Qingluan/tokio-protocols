...
where W: AsyncWriteExt + std::marker::Unpin + AsyncReadExt
{
    ...
    pub fn new(conv: u32, output: W) -> Self {
        Kcb {
            // state: 0,
            snd_una: 0,
            snd_nxt: 0,
    ...
    pub fn recv(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        for seg in &self.rcv_queue {
            std::io::Write::write_all(&mut buf, &seg.data)?;
            // std::io::write_all(&mut buf, &seg.data)?;
            index += 1;
            if seg.frg == 0 {
                break;
            }
    ...
    pub fn send(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        if self.stream {
            if let Some(seg) = self.snd_queue.back_mut() {
                ...
                if l < self.mss as usize {
                    ...
                    std::io::Read::read_exact(&mut buf, &mut seg.data[l..new_len])?;
        ...
        for i in 0..count {
            ...
            std::io::Read::read_exact(&mut buf, &mut seg.data)?;
    ...
    pub async fn update(&mut self, current: u32) {
